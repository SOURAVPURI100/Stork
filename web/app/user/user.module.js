...
angular.module('stork.user', [
  'ngCookies', 'stork', 'ngRoute', 'ui.bootstrap.modal'
])
    ...
    .service('user', function (stork, $location, $rootScope, $cookies) {
        ...
        this.login = function (info) {
            console.log("Login user called"); // ++ spu
            console.log("info " + info); // ++ spu
            if (info) {
    ...
    .controller('User', function ($scope, $modal, $location, user, stork, $rootScope) {
            ...
            $rootScope.location = $location; // ++ spu
            $rootScope.ad = false;
            ...
            /* Log the user out. */
            //  $scope.logout = function () { -- spu
            // Ins Begin of ++ spu
            $scope.logout = function () {
                $rootScope.modalInstance = $modal({
                    contentTemplate: '/app/user/logout.html',
        ...
        //  Ins Begin of ++ spu
        $scope.logoutConfirm = function (isConfirm) {
            if (isConfirm == 'Yes') {
                user.forgetLogin();
                $location.path('/');
                $modal({
